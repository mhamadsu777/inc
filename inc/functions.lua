...
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(monster..":NameBot:")
...
var = 'مطور البوت 👨🏽‍💻' 
elseif redis:sismember(monster..':KARA_BOT:'..ChatID,UserID) then
var = ' المنشىء الاساسي👷🏽' 
...
EnterCmd = true
local NumberFlood = tonumber(redis:get(monster..':Flood_Spam:'..msg.sender_user_id_..':msgs') or 0)
if NumberFlood > 3 then 
redis:setex(monster..':Flood_Spam:'..msg.sender_user_id_..':Stoped',10,true)
EnterCmd = false
elseif redis:get(monster..':Flood_Spam:'..msg.sender_user_id_..':Stoped') then
EnterCmd = false
else
redis:setex(monster..':Flood_Spam:'..msg.sender_user_id_..':msgs',3,NumberFlood+1)
end
return EnterCmd
end
...
monster..'num_msg_monster'..id_group,
...
redis:hset(monster..'username:'..data.id_,'username',ResolveUser(data))
redis:sadd(monster..'admins:'..msg.chat_id_,data.id_)
end,nil)
end
end
if NumAdmin == 0 then 
...
if redis:get(monster..'group:add'..msg.chat_id_) then  return '🎗*│*المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
redis:setex(monster..":ismember:"..msg.sender_user_id_,1500,true)
...
local lock_service = redis:get(monster..'lock_service')
...
GetUserID(v.user_id_,function(arg,data)
redis:hset(monster..'username:'..data.id_, 'username', ResolveUser(data))
redis:sadd(monster..':KARA_BOT:'..msg.chat_id_,data.id_)
end)
...
redis:mset(
monster..'group:add'..msg.chat_id_,true,
monster..'lock_link'..msg.chat_id_,true, 
monster..'lock_id'..msg.chat_id_,true,
monster..'lock_spam'..msg.chat_id_,true,
monster..'lock_webpage'..msg.chat_id_,true,
monster..'lock_markdown'..msg.chat_id_,true,
monster..'lock_flood'..msg.chat_id_,true,
...
monster..'mute_keyboard'..msg.chat_id_,true,
monster..'mute_game'..msg.chat_id_,true,
monster..'mute_inline'..msg.chat_id_,true,
monster..'lock_username'..msg.chat_id_,true,
monster..'num_msg_monster'..msg.chat_id_,5, 
monster..'replay'..msg.chat_id_,true
)

redis:sadd(monster..'group:ids',msg.chat_id_) 
redis:sadd(monster..'mtwr_count'..msg.sender_user_id_,msg.chat_id_)

GroupTitle(msg.chat_id_,function(arg,data)
local NameGroup = data.title_
redis:set(monster..'group:name'..msg.chat_id_,NameGroup)
...
redis:set(monster..'linkGroup'..msg.chat_id_,Gp_Link)
...
if not redis:get(monster..'group:add'..msg.chat_id_) then return '🛠*│* المجموعه بالتأكيد ✓️ تم تعطيلها' end  
...
if redis:sismember(monster..':KARA_BOT:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂╿العضو » ❪ '..USERNAME..' ❫\n🎫│الايدي » ❪ '..UserID..' ❫\n💯╽انه بالتأكيد منشىء اساسي \n✓️',17,USERCAR) 
end
redis:hset(monster..'username:'..UserID,'username',Resolv)
redis:sadd(monster..':KARA_BOT:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂╿العضو » ❪ '..USERNAME..' ❫\n🎫│الايدي » ❪ '..UserID..' ❫\n💯╽تمت ترقيته ليصبح منشىء اساسي في البوت \n✓️',17,USERCAR) 
end

if cmd == "remkara" then
if not redis:sismember(monster..':KARA_BOT:'..ChatID,UserID) then
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂╿العضو » ❪ '..USERNAME..' ❫\n🎫│الايدي » ❪ '..UserID..' ❫\n💯╽انه بالتأكيد ليس منشىء اساسي\n✓️',17,USERCAR) 
end
redis:srem(monster..':KARA_BOT:'..ChatID,UserID)
...
if cmd == "unban" then
if not redis:sismember(monster..'banned:'..ChatID,UserID) then 
...
if redis:sismember(monster..'is_silent_users:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂╿العضو » ❪ '..USERNAME..' ❫\n🎫│الايدي » ❪ '..UserID..' ❫\n💯╽تم بالتأكيد كتمه  \n✓️',17,USERCAR) 
end
redis:hset(monster..'username:'..UserID, 'username', Resolv)
...
if not redis:sismember(monster..'is_silent_users:'..ChatID,UserID) then 
...
if redis:sismember(monster..'banned:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'🙍🏻‍♂*╿*العضو » ❪ '..UserName..' ❫\n🎫*│*الايدي » ❪ `'..UserID..'` ❫\n💯*╽*تم بالتأكيد حظره \n✓️') 
end
kick_user(UserID, ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
...
if cmd == "dn_sudo" then
if not redis:sismember(monster..':SUDO_BOT:',UserID) then return sendMsg(ChatID,MsgID,'🙍🏻‍♂*╿*العضو » '..USERNAME..' \n🎫*│* الايدي » (`'..UserID..'`)\n💯*╽*انه بالتأكيد ليس مطور \n✓') end
redis:srem(monster..':SUDO_BOT:',UserID) 
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂╿العضو » ❪ '..USERNAME..' ❫\n🎫│الايدي » ❪ '..UserID..' ❫\n💯╽تم تنزيله من المطورين \n✓️',17,USERCAR) 
end
...
if redis:get(monster..'welcome:get'..msg.chat_id_) then welcomex = "❪ مفعل ❫" else welcomex = "❪ معطل ❫" end
if redis:get(monster..'lock_id'..msg.chat_id_) then lock_idx = "❪ مفعل ❫" else lock_idx = "❪ معطل ❫" end
if redis:get(monster..'replay'..msg.chat_id_)  then replayx  = "❪ مفعل ❫" else replayx  = "❪ معطل ❫" end
if redis:get(monster..'lock_woring'..msg.chat_id_) then lock_woringx = "❪ مفعل ❫" else lock_woringx  = "❪ معطل ❫" end
...
.."\n⃣│التاك «» "..(redis:get(monster..'lock_tag'..msg.chat_id_) or 'false')
...
.."\n🏌🏻│بالطرد البوتات «» "..(redis:get(monster..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n🤖│البوتات «» "..(redis:get(monster..'lock_bots'..msg.chat_id_) or 'false')
.."\n➕│عدد التكرار «» "..(redis:get(monster..'num_msg_monster'..msg.chat_id_) or 'false')
...
.."\n🎙│البصمات «» "..(redis:get(monster..'mute_audio'..msg.chat_id_) or 'false')
.."\n🔉│الصوت «» "..(redis:get(monster..'mute_voice'..msg.chat_id_) or 'false')
